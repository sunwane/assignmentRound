...
export const useHomeData = () => {
  ...
  return useMemo(() => {
    const genres = enrichGenresWithBookCounts(genreDefinitions, books);
    const recommendedBooks = getBooksByIds(books, [
      "book-midnight-library",
      "book-thursday-murder-club",
      "book-project-hail-mary",
      "book-braiding-sweetgrass",
      "book-little-life"
    ]).map(book => ({ ...book, badge: "featured" }));

    const newBooks = getBooksByIds(books, [
      "book-tomorrow-tomorrow",
      "book-guest-list",
      "book-wild-robot",
      "book-atomic-habits",
      "book-sun-and-her-flowers"
    ]).map(book => ({ ...book, badge: "new" }));

    const bestsellerBooks = getBooksByIds(books, [
      "book-dune",
      "book-silent-patient",
      "book-sapiens",
      "book-normal-people",
      "book-matilda",
    ]).map(book => ({ ...book, badge: "bestseller" }));
